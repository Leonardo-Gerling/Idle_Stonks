...
export default function Shop(): ReactElement {
    ...
    return (
        <div aria-label="shop" className="grid grid-rows-5 divide-y divide-dotted ml-6 select-none max-w-[20vw] top-1/2 -translate-y-1/2 absolute">
            {["Shop"].map(elem => <ShopTile itemName={"Farmer"} initialPrice={15} setPriceMethod={setFarmerPrice}>{elem}</ShopTile>)}
            {["Shop"].map(elem => <ShopTile itemName={"Backyard Tree"} initialPrice={200} setPriceMethod={setBackyardTreePrice}>{elem}</ShopTile>)}
            {["Shop"].map(elem => <ShopTile itemName={"Apple Farm"} initialPrice={7000} setPriceMethod={setAppleFarmPrice}>{elem}</ShopTile>)}
        </div>
    )
}
